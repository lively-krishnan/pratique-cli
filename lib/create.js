...
const { emptyDir, copy, mkdirSyncs } = require('./util/file-method')
const exec = require("./util/exec")
const spinner = require('./util/ora')
const cwd = process.cwd()
const log = console.log

async function init(projectName,dirs,options) {
  console.log(projectName,'+',dirs[0],options);
  //  模板名称  
  ...
  const dirsPath = dirs[0] ? `${dirs[0]}/${projectName}` : projectName
  
  const root = path.join(cwd, dirsPath)
